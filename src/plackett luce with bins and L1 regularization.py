...
def pl_neg_log_likelihood_l1(beta, races, lambda_l1=1.0):
    ll = 0.0
    ...
    penalty = lambda_l1 * np.sum(np.abs(beta))
...
p = len(predictors)
beta0 = np.zeros(p)
lambda_l1 = 0.1  # start small

res = minimize(
    pl_neg_log_likelihood_l1,
    beta0,
    args=(races, lambda_l1),
    method="BFGS"
)
...
# a) Log-likelihood
loglik = -pl_neg_log_likelihood_l1(beta_hat, races, lambda_l1)
...
with pd.ExcelWriter("PL_Race_Level_Probabilities_2024(5).xlsx", engine="openpyxl") as writer:
